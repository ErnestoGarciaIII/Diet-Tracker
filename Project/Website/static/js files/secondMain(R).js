...
function selectGoal(element, goal) {
    document.querySelectorAll('.goalCard').forEach(card => card.classList.remove('selected'));
...
async function nextStep2() {
    ...
    const goalString = localStorage.getItem('selectedGoal'); // From your selectGoal() function
    
    const goalMap = { 'lose': 2, 'maintain': 1, 'gain': 3 };
    const goalInt = goalMap[goalString] || 1;

    try {
        const response = await fetch(`http://127.0.0.1:5000/update-goal/${userId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ goal: goalInt })
        });

        if (response.ok) {
            const data = await response.json();
            // Save final goal for the airplane dashboard
...
// --- HELPER: Map Activity Strings to Python Integers ---
function mapActivityToInteger(level) {
    const levels = {
        'sedentary': 1,
        'light': 2,
        'moderate': 3,
...
function logFood() {
    ...
    const kCal = document.getElementById('calInput');
    const macros = document.getElementById('macrosInput')?.value || 0;
    const micros = document.getElementById('microsInput')?.value ||0;
    
    const foodName = foodEl.value;
    const calories = parseInt(kCal.value);
    const Macro = parseInt(macros.value);
    const Micro = parseInt(micros.value);
...
function logFoodToDatabase(name, kcal) {
